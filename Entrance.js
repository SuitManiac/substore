 ...
 * - [format] 自定义格式, 从 节点(proxy) 和 入口(api)中取数据. 默认为: {{api.country}} {{api.regionName}}
...
async function operator(proxies = [], targetPlatform, context) {
  ...
  let format = $arguments.format || `{{api.country-api.regionName}} {{api.regionName}}`; // 自定义格式
  ...
  if (internal) {
    ...
    format = $arguments.format || `{{proxy.name}}-{{api.regionName}}`; // 使用内部方法时的默认格式
