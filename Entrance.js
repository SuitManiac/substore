...
async function operator(proxies = [], targetPlatform, context) {
  ...
  let format = $arguments.format || `{{proxy.name}} {{api.regionName}} -{{api.regionName}}`; // 自定义格式
  ...
  if (internal) {
    ...
    format = $arguments.format || `{{proxy.name}} {{api.regionName}}-{{api.regionName}}`; // 使用内部方法时的默认格式
